...
def get_point_cloud(B_matrix, parallax_map, color_map, poses, mask_lower_bound=2):
    ...
    o3d.io.write_point_cloud(OP1_DIR+"/point_cloud_{}.ply".format(datetime.datetime.now().strftime("%d-%b-%Y %H:%M:%S.%f")), pcd)
...
def get_image(i, point_cloud, point_cloud_colors, R, T, K, im_width, im_height):
    ...
    cv2.imwrite(OP1_DIR+"/reconstructed_images"+"/reconstructed_image_{}.png".format(i), cv2.cvtColor(image, cv2.COLOR_RGB2BGR))
    return image

def image_point_cloud(point_cloud, point_cloud_colors, poses, image_width, image_height):
    bar = FillingSquaresBar('Imaging the Point Cloud with the given poses', max=len(poses))
    for i in range(len(poses)):
        P = poses[i]
        R = P[:,:3]
        T = -1*(R.T @ P[:,3])
        R = R.T
        im = get_image(i, point_cloud, point_cloud_colors, R, T, K, image_width, image_height)
        bar.next()
    bar.finish()
